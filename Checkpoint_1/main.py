...
def robotControl(trigger, simultaneous_launcher, motor_command_queue, sensor_queue):
    ...
    try:
        ...
        dist_servo = gopigo3_robot.init_servo("SERVO1")
        cam_servo = gopigo3_robot.init_servo("SERVO2")
        dist_servo.reset_servo()
        cam_servo.reset_servo()
    except IOError:
    ...
    while not (trigger.is_set() or simultaneous_launcher.broken or gopigo3_robot.volt() <= MINIMUM_VOLTAGE):
        ...
        if command == "west":
        ...
        elif command == "distServoLeft":
            dist_servo.rotate_servo(0)
        elif command == "distServoRight":
            dist_servo.rotate_servo(180)
        elif command == "camServoLeft":
            cam_servo.rotate_servo(0)
        elif command == "camServoRight":
            cam_servo.rotate_servo(180)
            ...
            with picamera.PiCamera() as camera:
                ...
                # Camera warm-up time
                sleep(1)
                img_name = "capture" + str(img_num) + ".jpg"
...
def Main(trigger):
    ...
    available_commands = {"<LEFT>": "west",
                          ...
                          ".": "camServoLeft",
                          "/": "camServoRight",
                          "1": "capture"} # the selectable options within the menu
    menu_order = ["<LEFT>", "<RIGHT>", "<UP>", "<DOWN>", "<SPACE>", "w", ";", "'", ".", "/", "1"] # and the order of these options
